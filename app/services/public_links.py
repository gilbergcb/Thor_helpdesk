...
from app.core.security import (
    generate_public_ticket_code,
    generate_public_ticket_token,
    hash_public_token,
)
...
class PublicTicketLinkService:
    ...
    def create_for_ticket(self, ticket: Ticket) -> str:
        ...
        self.db.add(
            TicketPublicLink(
                ...
                code_hash=hash_public_token(code),
            )
        )
        return token

    def create_short_for_ticket(self, ticket: Ticket) -> str:
        token = generate_public_ticket_token()
        code = self._unique_code()
        self.db.add(
            TicketPublicLink(
    ...
    def public_short_url(self, ticket: Ticket, code: str) -> str:
        base_url = str(get_settings().public_app_url).rstrip("/")
        return f"{base_url}/c/{ticket.protocol}/{code}"
    ...
    def get_valid_code_link(self, code: str) -> TicketPublicLink | None:
        now = datetime.now(UTC)
        link = self.db.scalar(
            select(TicketPublicLink)
            .where(TicketPublicLink.code_hash == hash_public_token(code))
            .options(
                joinedload(TicketPublicLink.ticket).joinedload(Ticket.client),
                joinedload(TicketPublicLink.ticket).joinedload(Ticket.whatsapp_group),
                joinedload(TicketPublicLink.ticket)
    ...
    def _unique_code(self) -> str:
        for _ in range(5):
            code = generate_public_ticket_code()
            exists = self.db.scalar(
                select(TicketPublicLink.id).where(
                    TicketPublicLink.code_hash == hash_public_token(code)
                )
            )
            if exists is None:
                return code
